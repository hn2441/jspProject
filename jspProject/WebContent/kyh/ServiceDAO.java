...
public class ServiceDAO {
	...
	public void update(ServiceDTO dto) throws Exception {
		...
		// 3. SQL문 객체화
		String sql = "update board set title = ?, pw = ?, content = ? where email = ? ";
	...
	public void insert(ServiceDTO dto) throws Exception {
		...
		// 3. SQL등록쿼리문
		String sql = "insert into board(title,email,pw,content) values (?,?,?,?)";
		//String sql2="insert into board(null,?,?,?,?,null)values";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setString(1, dto.getTitle());
...
/*	public int getNext() { // 게시글의 번호 다음 페이지 함수
	...
	public ArrayList<ServiceDTO> selectAll() throws Exception {
		...
		// 3. SQL문 객체화
		// String sql = "select *from board ORDER BY boardID desc limit 10";
	...
	/*public ArrayList<ServiceDTO> getList(int pageNumber) {
		...
		try {
			...
			while (rs.next()) {
				...
				service.setBoardID(rs.getInt(1));
